...
export function registerTools(server: McpServer) {
  ...
  server.tool(
    ...
    {
      query: z.string().describe("The SQL statement(s) to execute. Separate multiple statements with semi-colons"),
      defaultSchema: z.string().optional().describe("Schema to use if tables are not prefixed with a schema name"),
      schemaOnly: z.boolean().optional().describe("If true, the result only includes column metadata"),
      parameters: z.record(z.any()).optional().describe("A JSON object containing a list of query parameters. All parameter names must begin with @"),
    },
  ...
  server.tool(
    ...
    "Execute batch operations (INSERT, UPDATE, DELETE) against connected data sources",
    {
      ...
      defaultSchema: z.string().optional().describe("Schema to use if tables are not prefixed with a schema name"),
      parameters: z.array(z.record(z.any())).optional().describe("An array of parameter set objects where each set represents parameters for a single item in the batch"),
    },
  ...
  server.tool(
    ...
    "Execute stored procedures against connected data sources",
    {
      ...
      defaultSchema: z.string().optional().describe("Schema to use if the procedure is not prefixed with a schema name"),
      parameters: z.record(z.any()).optional().describe("A JSON object containing procedure parameters. All parameter names must begin with @"),
    },
  ...
  server.tool(
    ...
    "Retrieve a list of available stored procedure parameters from CData Connect Cloud for a specific catalog, schema, and procedure",
    {
      ...
      procedureName: z.string().optional().describe("Optional procedure name to filter parameters by"),
